      ...
      <form onSubmit={handleSubmit} className="rounded-2xl bg-white p-7 shadow ring-1 ring-black/5">
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <input type="number" min="0" step="1" value={totalBudget} onChange={(e)=>setTotalBudget(e.target.value)} placeholder="Overall monthly budget" className="w-full rounded-xl border border-gray-300 px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
        </div>
        ...
        <div className="mt-1 flex items-center justify-end">
          {Number(totalBudget) > 0 ? (
            <Tag color={totalAllocated > Number(totalBudget) ? 'rose' : 'emerald'} size="sm">
              {totalAllocated > Number(totalBudget)
                ? `Exceeded by $${(totalAllocated - Number(totalBudget)).toFixed(2)}`
                : `Remaining: $${(Number(totalBudget) - totalAllocated).toFixed(2)}`}
            </Tag>
          ) : (
            <Tag color="gray" size="sm">No total budget set</Tag>
          )}
        </div>

        <button type="submit" className="mt-6 w-full rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white shadow hover:bg-blue-700">Update</button>
      </form>
    </div>
  );
}
